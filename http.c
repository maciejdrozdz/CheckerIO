...
#include "checkerIO/win_checkerio.cpp"
...
#define MAX_TAG_VALUE_SIZE 6
#define MOVE_PTR_TO_VALUE_START(Content, TagName) (Content + ((sizeof(TagName) - 2) + sizeof(TAG_SEPARATOR)))
...
tag_value_result GetValueAfterTag(char *Content)
{
	tag_value_result Result;
	Result.Size = 0;

	char CharValue[MAX_TAG_VALUE_SIZE + 1];
	char *ValuePtr = CharValue;
	char *C = Content;
	while (*C != '\r' && *C != '\n' && *C != '\t' && Result.Size++ < MAX_TAG_VALUE_SIZE)
	{
		*ValuePtr++ = *C++;
	}
	*ValuePtr = '\0';
	...
	strncpy(Result.Content, CharValue, Result.Size);

	return(Result);
}

static tag_value_result MessageResult[3];
static i32 MessageContentCounter = 0;

b32 ParseMultipartContent(char *Content)
{
	b32 Result = false;

	tag_value_result *FormResult = MessageResult + MessageContentCounter;
	if (!strncmp(Content, TAG_FROM, sizeof(TAG_FROM) - 2))
	{
		*FormResult = GetValueAfterTag(MOVE_PTR_TO_VALUE_START(Content, TAG_FROM));
		FormResult->Tag = VALUE_FROM;
		MessageContentCounter++;
	}
	else if (!strncmp(Content, TAG_TO, sizeof(TAG_TO) - 2))
	{
		*FormResult = GetValueAfterTag(MOVE_PTR_TO_VALUE_START(Content, TAG_TO));
		FormResult->Tag = VALUE_TO;
		MessageContentCounter++;
	}
	else if (!strncmp(Content, TAG_ID, sizeof(TAG_ID) - 2))
	{
		*FormResult = GetValueAfterTag(MOVE_PTR_TO_VALUE_START(Content, TAG_ID));
		FormResult->Tag = VALUE_ID;
		MessageContentCounter++;
	}

	if (MessageContentCounter >= ArrayCount(MessageResult))
	{
		Result = true;
	}
			...
			for (i = 0; i < hp->maxClients; i++) {
				...
				if ((bRead|bWrite)!=0) {
					...
					if (bWrite && ISFLAGSET(phsSocketCur,FLAG_SENDING)) {
						...
						iRc=_mwProcessReadSocket(hp,phsSocketCur, &Game, &GameMemory);
					} else {
						iRc=-1;
...
////////////////////////////////////////////////////////////////////////////
int _mwProcessReadSocket(HttpParam* hp, HttpSocket* phsSocket, win32_game_code *GameCode, game_memory *GameMemory)
{
	...
	request CheckerIORequest = { };
	{
		...
		if (iLength <= 0) {
			...
			if (ISFLAGSET(phsSocket,FLAG_REQUEST_POST) && phsSocket->pxMP && hp->pfnFileUpload) {
				...
				if (!pxMP->pchBoundaryValue[0]) {
					...
					(hp->pfnFileUpload)(phsSocket->pxMP, (OCTET *)phsSocket->pucData, (int)phsSocket->dataLength);
				}
				_mwCloseSocket(hp, phsSocket);
				return 0;
			}
		...
		// Check if this is checkerIO request
		char *CheckerIOStart = strstr(phsSocket->pucData, "checkerIO&&");
		if (CheckerIOStart)
		{
			int MessageLen = strlen(CheckerIOStart);
			if (MessageLen <= 128) // Check for oversized messages
			{
				char *CheckerIOMessage = (char *)malloc(MessageLen);
				memcpy(CheckerIOMessage, CheckerIOStart, MessageLen);
				CheckerIORequest = { CheckerIOMessage, MessageLen };
			}
		}
	}
			...
			if (ISFLAGSET(phsSocket,FLAG_REQUEST_POST)) {
				if (phsSocket->pxMP) {
					...
					if (ISFLAGSET(phsSocket, FLAG_MULTIPART)) {
						...
						if (hp->pfnFileUpload) (hp->pfnFileUpload)(pxMP, (OCTET *)phsSocket->buffer + phsSocket->request.headerSize, phsSocket->dataLength);
    ...
    if (ISFLAGSET(phsSocket,FLAG_REQUEST_POST) && phsSocket->pxMP) {
		if (!phsSocket->pxMP->pchBoundaryValue[0]) {
			...
			if (phsSocket->dataLength == phsSocket->bufferSize && hp->pfnFileUpload) {
				(hp->pfnFileUpload)(phsSocket->pxMP, (OCTET *)phsSocket->pucData, phsSocket->dataLength);
				phsSocket->dataLength = 0;
			}
		}
		return 0;
	...
	int CheckerIOMessageLen = 0;
	if (CheckerIORequest.Length)
	{
		//SETFLAG(phsSocket, FLAG_CHECKERIO);
		thread_context Thread = {};
		response Response = {};

		if (GameCode->UpdateAndRender)
		{
			GameCode->UpdateAndRender(&Thread, GameMemory, CheckerIORequest, &Response);
		}
		//send(phsSocket->socket, , , 0);

		SETFLAG(phsSocket, FLAG_DATA_RAW);
		phsSocket->pucData = (char *)Response.Data;
		phsSocket->dataLength = Response.Size;
		//phsSocket->response.contentLength = Response.Size;
		DBG("URL handler: raw data\n");
	}
	else
	{
	...
	/*if (ISFLAGSET(phsSocket, FLAG_CHECKERIO)) {
		
		return 1;//_mwStartSendRawData(hp, phsSocket);
	}
...
int _mwParseHttpHeader(HttpSocket* phsSocket)
{
	...
	for(;;) {
		...
		} else if (_mwStrHeadMatch(&p, HTTP_MULTIPARTCONTENT)) {
			ParseMultipartContent(p);
